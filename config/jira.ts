...
async function issueFieldsTranslator(
  config: GramConfiguration,
  jiraEnvironment: "sandbox" | "production",
  dal: DataAccessLayer,
  actionItem: Threat,
  ...
  const componentName =
    model?.data.components.find((c) => c.id === actionItem.componentId)?.name ||
    "unknown component";
...
export function createJiraActionItemExporter(
  ...
  const jiraActionItemExporterConfig: JiraActionItemExporterConfig = {
    ...
    transitionOnUpdateExistingIssue: "21",
  };
