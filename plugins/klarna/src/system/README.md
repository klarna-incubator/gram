...
```ts
const systemRegistrySystemProvider = new SystemRegistrySystemProvider();
const j1SystemProvider = new KlarnaSystemProvider(
  registrySystemProvider,
  j1ClientFactory
);
// …
return { systemProvider: [j1SystemProvider, systemRegistrySystemProvider], ... };
```
...
- **System Registry API / systems:**  
  [Internal System Registry API](https://wiki.klarna.net/wiki/System_Registry_Api_(API_Spec))
