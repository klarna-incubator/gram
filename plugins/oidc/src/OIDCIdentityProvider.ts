...
import { aes256gcm, extractGroups } from "./util.js";
...
export class OIDCIdentityProvider implements IdentityProvider {
  ...
  constructor(
    ...
    private captureGroupClaims: boolean = true,
    private groupsClaimName: string = "groups"
  ) {
  ...
  async getIdentity(ctx: RequestContext): Promise<LoginResult> {
    ...
    const sub = payload[this.fieldForIdentitySub] as string;

    // Capture and store user info and groups if enabled
    if (this.captureGroupClaims) {
      // Store simplified user info focusing on authorization needs
      const userInfo: OIDCUserInfo = {
        sub, // Ensure we use the correct sub field
        email: payload.email as string,
        name: payload.name as string,
        groups: extractGroups(payload, this.groupsClaimName),
      };

      const userStore = OIDCUserStore.getInstance(this.dal);
      await userStore.storeUser(userInfo);
      log.debug(`Stored user info for user ${sub}`, {
